...
def print_death_age(people_dict):
    ...
    Return: nothing
    """
    for element in people_dict:
        name = people_dict[element][0]
        birth = people_dict[element][2]
        death = people_dict[element][3]
        age = death - birth
...
def count_genders(people_dict):
    ...
    female = 0
    for element in people_dict:
        gender = people_dict[element][1]
        if gender == "M":
            male += 1

        else:
            female +=1
    print(f"There are {male} males and {female} females!")
...
def print_marriages(marriages_dict, people_dict):
    ...
    Return: nothing
    """
    for element in marriages_dict:
        name_husband = marriages_dict[element][0]
        name_wife = marriages_dict[element][1]
        marriage_year = marriages_dict[element][2]
        male_name = people_dict[name_husband][0]
        female_name = people_dict[name_wife][0]
        husband_birth_year = people_dict[name_husband][2]
        wife_birth_year = people_dict[name_wife][2]
        
        husband_marriage_age = marriage_year - husband_birth_year
        wife_marriage_age = marriage_year - wife_birth_year
        print(f"{male_name} was {husband_marriage_age} when he was married to {female_name} who was {wife_marriage_age} in the year {marriage_year}")
